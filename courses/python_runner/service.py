...
from .notifications import build_python_system_notification


def validate_python_activity_submission(lesson, response):
    ...
    if not submission:
        result = ActivityEvaluationResult(
            is_correct=False,
            title=ERROR_MESSAGE,
            explanation="Your submission is empty. Write your Python code before checking the activity.",
            execution_status="error",
            used_code_runner=True,
            language="python",
            details={"stage": "execute"},
        )
        result.notification = build_python_system_notification(result=result)
        return result
    ...
    if len(submission.encode("utf-8")) > max_source_bytes:
        result = ActivityEvaluationResult(
            is_correct=False,
            title=ERROR_MESSAGE,
            explanation="Your Python code is too large for this lesson runner to check safely.",
            execution_status="error",
            used_code_runner=True,
            language="python",
    ...
    blocked_construct = find_blocked_construct("python", submission)
    if blocked_construct:
        result = ActivityEvaluationResult(
            ...
            learning_suggestion="Stay with the lesson's core Python features until the runner accepts the solution.",
            details={"stage": "execute"},
        )
        result.notification = build_python_system_notification(result=result)
        return result
    ...
    if source_issues or not output_match:
        ...
        result = ActivityEvaluationResult(
            ...
            or "Your Python code ran, but it does not meet the lesson requirements yet.",
            execution_status=execution_result.execution_status,
            program_output=execution_result.program_output,
            errors=execution_result.errors,
    ...
    result = ActivityEvaluationResult(
        ...
        explanation=rules.get("success_explanation") or "Your Python code executed successfully.",
        execution_status=execution_result.execution_status,
        program_output=execution_result.program_output,
        errors=execution_result.errors,
...
def _output_matches(rules, program_output):
    ...
    if expected_patterns:
        ...
        if missing_patterns:
            return False, "The output is missing one or more expected Python results."
    ...
    if expected_contains:
        ...
        if missing_fragments:
            return False, "Make sure the Python output includes the expected text shown in the lesson."
...
def _default_learning_suggestion():
    return "Trace the Python program from top to bottom and check what should happen before and after each line."
