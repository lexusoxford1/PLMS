...
def validate_code_activity_submission(lesson, response):
    ...
    if source_issues or not output_match:
        explanation = rules.get("incorrect_explanation") or "Your code ran, but it did not meet all of the lesson requirements yet."
        hints = []
        if source_issues:
...
def _output_matches(rules, program_output):
    ...
    if expected_patterns:
        ...
        if missing_patterns:
            return False, "The output is missing one or more expected patterns."
    ...
    if expected_contains:
        ...
        if missing_fragments:
            return False, "Make sure the output includes the expected text shown in the activity."
...
def _default_learning_suggestion(language):
    label = language_label(language)
    return f"Run through the {label} lesson example again and trace what each line should print before you submit the next attempt."
